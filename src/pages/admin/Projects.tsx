  ...
  return (
    <div className="flex-1 min-h-0 overflow-y-auto">
      <div className="p-4 space-y-4 max-w-2xl mx-auto pb-8">
        ...
        {banner && (
          <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-3 text-xs whitespace-pre-wrap">
            <div className="flex items-start justify-between gap-2">
              <span className="min-w-0 break-words">{banner}</span>
              <button onClick={() => setBanner(null)} className="text-zinc-500 hover:text-zinc-200">✕</button>
            </div>
          </div>
        )}
            ...
            <li key={p.id} className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-3">
              <div className="flex items-start justify-between gap-2">
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  {/* Same handlers as the chat three-dot menu — see lib/projectActions.ts */}
                  <ProjectActionsMenu
                    actions={buildProjectActions({
                      masterSecret: session?.masterSecret ?? "",
                      resolveProject: async () => ({ id: p.id, name: p.name }),
                      report: (msg) => setBanner(msg),
                      onChanged: load,
                    })}
                  />
                </div>
